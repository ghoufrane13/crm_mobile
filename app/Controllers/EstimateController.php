...
class EstimateController extends ResourceController
{
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // GET /api/estimates/list?staff_id=X&status=Y
    // ─────────────────────────────────────────────────────────────────────────
    public function list()
    {
        ...
        foreach ($estimates as &$e) {
            $s = (int)$e['status'];
            $e['status_label'] = $this->statuses[$s] ?? 'Inconnu';
        }
    ...
    public function create()
    {
        ...
        $estimateData = [
            ...
            'expirydate'        => $data['expirydate'] ?? date('Y-m-d', strtotime('+30 days')),
        ...
        return $this->respond([
            'status'           => true,
            'message'          => "Devis $formattedNumber créé",
     ...
     public function update($id = null)
    {
        ...
        // Bloquer la modification si déjà Accepté (sauf force=true)
        if ((int)$existing['status'] === 4 && empty($data['force'])) {
            return $this->respond([
                'status'  => false,
                'message' => 'Devis accepté — modification impossible. Passez force=true pour forcer.',
            ], 400);
        }

        $u = [];

        foreach (['date', 'expirydate', 'reference_no', 'discount_type', 'terms',
                  'billing_street', 'billing_city', 'billing_state', 'billing_zip',
                  'shipping_street', 'shipping_city', 'shipping_state', 'shipping_zip',
                  'clientnote', 'adminnote'] as $f) {
            if (array_key_exists($f, $data)) $u[$f] = $data[$f];
        }

        foreach (['currency', 'clientid', 'billing_country', 'shipping_country',
                  'include_shipping', 'sale_agent', 'status'] as $f) {
            if (array_key_exists($f, $data)) $u[$f] = (int)$data[$f];
        }

        foreach (['subtotal', 'total_tax', 'total', 'adjustment',
                  'discount_percent', 'discount_total'] as $f) {
            if (array_key_exists($f, $data)) $u[$f] = (float)$data[$f];
        }

        if (!empty($u)) {
            $this->db->table('tblestimates')->where('id', (int)$id)->update($u);
        }

        // Remplacer les articles si fournis
    ...
    // Supprime le devis + ses articles (tblitemable) en cascade
    // Règle : un devis Accepté (status=4) ou déjà converti (invoiceid != null)
    ...
    public function delete($id = null)
    {
        ...
        $estimate = $this->db->table('tblestimates')
            ->select('id, status, invoiceid')
            ->where('id', (int)$id)
            ->get()->getRowArray();

        if (!$estimate) {
            return $this->respond(['status' => false, 'message' => 'Devis introuvable'], 404);
        }

        // Blocage si déjà converti en facture
        if (!empty($estimate['invoiceid'])) {
            return $this->respond([
                'status'  => false,
                'message' => 'Ce devis a été converti en facture et ne peut pas être supprimé.',
            ], 400);
        }

        // Blocage si statut Accepté (4)
        if ((int)$estimate['status'] === 4) {
            return $this->respond([
                'status'  => false,
                'message' => 'Un devis accepté ne peut pas être supprimé.',
            ], 400);
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // POST /api/estimates/change-status
    // Body: { estimate_id, status }
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public function sendEmail($id = null)
    {
        ...
        $sent = $this->_sendEstimateEmail($toEmail, $clientName, $staffName, (int)$id, $estimate);
        if (!$sent) return $this->respond(['status' => false, 'message' => 'Erreur envoi email'], 500);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // GET /api/estimates/pdf/:id
    // Retourne { status, pdf: base64, filename } pour affichage in-app
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // GET /api/estimates/pdf-download/:id
    // Retourne le PDF binaire avec Content-Disposition: attachment
    // Ouvrir directement dans un navigateur déclenche le téléchargement
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // POST /api/estimates/convert/:id
    // Convertit le devis en facture (tblinvoices) et marque le devis Accepté (4)
    // Body (optionnel): { staff_id, date, duedate, sale_agent,
    //                     billing_*, shipping_*, items[] }
    // ─────────────────────────────────────────────────────────────────────────
    public function convert($id = null)
    {
        ...
        if (!empty($estimate['invoiceid'])) {
            return $this->respond([
                ...
                'invoice_id' => (int)$estimate['invoiceid'],
            ], 400);
        }

        // Articles : ceux de Flutter si fournis, sinon ceux du devis
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // GET /api/estimates/client-list?contact_id=X
    // ─────────────────────────────────────────────────────────────────────────
    public function clientList()
    {
        ...
        $estimates = $this->db->table('tblestimates e')
            ->select('e.id, e.formatted_number, e.prefix, e.number, e.date, e.expirydate,
                      e.total, e.subtotal, e.status, e.reference_no,
        ...
        foreach ($estimates as &$e) {
            $e['status_label'] = $this->statuses[(int)$e['status']] ?? 'Inconnu';
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // GET /api/estimates/client-detail/:id?contact_id=X
    // ─────────────────────────────────────────────────────────────────────────
    public function clientDetail($id = null)
    {
        ...
        $estimate = $this->estimateModel->getDetail((int)$id);
        if (!$estimate || (int)$estimate['clientid'] !== (int)$contact['userid'])
            return $this->respond(['status' => false, 'message' => 'Devis introuvable'], 404);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // POST /api/estimates/client-respond/:id
    // Body: { contact_id, action: 'accept'|'decline' }
    // ─────────────────────────────────────────────────────────────────────────
    public function clientRespond($id = null)
    {
        ...
        if (!$contactId || !in_array($action, ['accept', 'decline']))
            return $this->respond(['status' => false, 'message' => 'Paramètres invalides'], 400);
        ...
        if (!$contact || !$estimate || (int)$estimate['clientid'] !== (int)$contact['userid'])
            return $this->respond(['status' => false, 'message' => 'Non autorisé'], 403);

        if ((int)$estimate['status'] !== 2)
            return $this->respond(['status' => false, 'message' => 'Ce devis ne peut plus être modifié'], 400);
        ...
        return $this->respond([
            'status'       => true,
            'message'      => 'Devis ' . ($action === 'accept' ? 'accepté' : 'décliné'),
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Crée une facture dans tblinvoices depuis un devis
    // ─────────────────────────────────────────────────────────────────────────
    private function _createInvoice(array $estimate, array $items, array $data): ?int
    {
        ...
        // Recalcul totaux depuis les articles
        $subtotal = 0.0;
        $totalTax = 0.0;
        ...
        $this->db->table('tblinvoices')->insert([
            ...
            'total'             => $total,
            'adjustment'        => (float)($estimate['adjustment'] ?? 0),
            'addedfrom'         => $staffId,
        ...
        // Copier les articles avec rel_type = 'invoice'
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Génère les bytes binaires du PDF via TCPDF Output('S')
    // ─────────────────────────────────────────────────────────────────────────
    private function _generatePdfBytes(array $estimate): string
    {
        $items      = $estimate['items'] ?? [];
        $sym        = $estimate['currency_symbol'] ?? '';
        $numStr     = $estimate['formatted_number'] ?? ('EST-' . str_pad($estimate['id'], 6, '0', STR_PAD_LEFT));
        $clientName = $estimate['client_company'] ?? $estimate['clientname'] ?? '';
        $agentName  = $estimate['sale_agent_name'] ?? '';

        $addressParts = array_filter([
            $estimate['billing_street'] ?? '',
            $estimate['billing_city']   ?? '',
            $estimate['billing_state']  ?? '',
            $estimate['billing_zip']    ?? '',
        ], fn($v) => trim($v) !== '');
        ...
        $pdf->SetXY($mL, $pdf->GetY());
        $pdf->Cell(0, 5, 'Signature autorisée  ________________________', 0, 1, 'L');

        return $pdf->Output('doc.pdf', 'S');
    }

    // ─────────────────────────────────────────────────────────────────────────
    // Envoi email via API Brevo HTTP (PDF en base64 → aucune corruption MIME)
    // ─────────────────────────────────────────────────────────────────────────
...
<div class='box'>
  <div class='hd'><h2>Devis $numStr</h2></div>
  <div class='bd'>
    <p>Bonjour <strong>" . htmlspecialchars($clientName) . "</strong>,</p>
    <p><strong>" . htmlspecialchars($staffName) . "</strong> vous a transmis un devis :</p>
    <p><strong style='font-size:16px'>$numStr</strong></p>
    <div class='note'>Le PDF de votre devis est joint à cet email.</div>
  ...
  <div class='ft'>© " . date('Y') . " — Envoyé automatiquement.</div>
        ...
        try {
            ...
            log_message('debug', 'PDF bytes: ' . strlen($pdfBytes) . ', base64 len: ' . strlen($pdfBase64));
        } catch (\Throwable $e) {
        ...
        log_message('debug', "Brevo API response [$httpCode]: $response");

        if ($curlErr) { log_message('error', 'Brevo cURL error: ' . $curlErr); return false; }
