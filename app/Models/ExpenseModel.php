...
class ExpenseModel extends Model
{
    ...
    // NOTE: next_expense_date is removed — this column does NOT exist in tblexpenses.
    // The actual schema columns are:
    //   id, category, currency, amount, tax, tax2, reference_no, note,
    //   expense_name, clientid, project_id, billable, invoiceid, paymentmode,
    //   date, recurring_type, repeat_every, recurring, cycles, total_cycles,
    //   custom_recurring, last_recurring_date, create_invoice_billable,
    //   send_invoice_to_customer, recurring_from, receipt, dateadded, addedfrom
    ...
    public function getExpenses(array $filters = []): array
    {
        ...
        if (!empty($filters['category']))  $builder->where('e.category', $filters['category']);
        if (!empty($filters['clientid']))  $builder->where('e.clientid', $filters['clientid']);
        if (!empty($filters['billable']))  $builder->where('e.billable', $filters['billable']);
        if (!empty($filters['date_from'])) $builder->where('e.date >=', $filters['date_from']);
        if (!empty($filters['date_to']))   $builder->where('e.date <=', $filters['date_to']);
    ...
    {
        return $this->db->table('tblexpenses_categories')
            ->select('id, name')
            ->orderBy('name', 'ASC')
            ->get()->getResultArray();
    }
    ...
    {
        $this->db->table('tblexpenses_categories')->insert(['name' => $name]);
